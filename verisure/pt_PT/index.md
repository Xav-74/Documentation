...
> **Importante**
>
> **ESTE PLUGIN NÃO ESTÁ, DE FORMA ALGUMA, ASSOCIADO OU LIGADO ÀS EMPRESAS DO GRUPO SECURITAS DIRECT - VERISURE.**
>
...
| **Tipo de alarme** | escolha do tipo de alarme (tipo 1 = Europa do Sul (França, Espanha, ...) / tipo 2 = Europa do Norte (Bélgica, Reino Unido, ...) / tipo 3 = nova geração (desde 2022)) |
| **Número de instalação**<br/>(alarmes de tipo 1 e 3) | indique o seu número de instalação Verisure. **Atenção! Este número deve ser exatamente idêntico ao que aparece na sua aplicação My Verisure. Se o seu número de instalação começar por um 0, mas este não estiver presente na aplicação, elimine-o!** |
| **Identificador**<br/>(alarmes de tipo 1, 2 e 3) | indique o seu nome de utilizador Verisure que utiliza para iniciar sessão no site [https://customers.securitasdirect.fr](https://customers.securitasdirect.fr) ou [https://mypages.verisure.com/](https://mypages.verisure.com) |
| **Palavra-passe**<br/>(alarmes de tipo 1, 2 e 3) | introduza a sua palavra-passe |
| **Código do alarme**<br/>(alarme tipo 2) | introduza o código PIN do seu alarme (4 ou 6 dígitos) |
| **País**<br/>(alarmes de tipo 1 e 3) | selecione o país onde o seu alarme está instalado (países atualmente suportados: França, Espanha, Grã-Bretanha, Itália, Portugal). Para os alarmes do tipo 2, a seleção do país é automática (países suportados atualmente: Bélgica, Países Baixos, Alemanha, Grã-Bretanha, Dinamarca, Finlândia, Noruega, Suécia) |

**Opções** (alarmes do tipo 1 e 3): dependendo do tipo do seu alarme, pode selecionar as seguintes opções:

| Opção | Descrição |
|---|---|
| **Atualização através do histórico**<br/>(alarmes do tipo 1 e 3) | permite atualizar os estados do alarme com base no histórico de ações. Não se esqueça de configurar e ativar o cron na configuração do plugin |
| **Alarme de presença exterior**<br/>(alarme tipo 3) | assinalar se tiver detetores exteriores e se o modo exterior estiver ativado no seu alarme |
| **Ativação forçada**<br/>(alarme tipo 3) | permite forçar a ativação do alarme mesmo que uma porta ou janela tenha ficado aberta. Por sua conta e risco! |
...
> **Atenção**
>
> Recomenda-se vivamente que crie, no seu espaço Verisure, um utilizador dedicado ao Jeedom com direitos de «administrador». O plugin suporta a autenticação multifator (MFA) para alarmes de tipo 1. O mesmo se aplica aos alarmes de tipo 2, mas recomenda-se desativar esta opção por enquanto, uma vez que a atualização do token é muito restritiva. Em caso de problema de ligação, o botão **Eliminar Token** permite eliminar os cookies guardados e reiniciar com uma autenticação inicial.
...
| Comando | Descrição |
|---|---|
| **Estado de ativação** | permite saber o estado de ativação do alarme<br/>**0**: desarmado<br/>**1**: armado |
| **Estado do alarme** | permite saber o estado do alarme<br/>**0**: estado normal<br/>**1**: alarme acionado |
| **Modo de alarme** | permite saber o modo de ativação do alarme<br/>**Modo total**: o alarme está ativado no modo total (alarmes de tipo 1, 2 e 3)<br/>**Modo noturno**: o alarme está ativado no modo noturno (alarme de tipo 1)<br/>**Modo diurno**: o alarme está ativado no modo diurno (alarme tipo 1)<br/>**Modo exterior**: o alarme está ativado no modo exterior (alarme tipo 1)<br/>**Modo parcial**: o alarme está ativado no modo parcial (alarme tipo 2 e 3) |
| **Qualidade da rede** | permite estimar a qualidade da rede 3G/4G dos alarmes do tipo 1 e 3 (com base no resultado das últimas 25 solicitações)<br/>ícone de 5 barras: nenhuma solicitação com erro nas últimas 25<br/>ícone de 4 barras: de 1 a 2 solicitações com erro nas últimas 25<br/>ícone de 3 barras: de 3 a 7 solicitações com erro nas últimas 25<br/>ícone de 2 barras: de 8 a 17 solicitações com erro nas últimas 25<br/>ícone de 1 barra: entre 18 e 24 pedidos com erro nos últimos 25<br/>ícone de 0 barras: 25 pedidos com erro nos últimos 25 |

> **Atenção**
>
> Nesta versão, o evento relacionado com o acionamento do alarme ainda não foi tido em conta!
...
| **Desativação** | desativa o alarme, independentemente do modo (alarmes de tipo 1, 2 e 3) |
...
> **Atenção**
>
> Por vezes, os comandos podem demorar vários segundos a serem executados (entre 15 e 25 segundos, ou mesmo mais de um minuto no caso de pedidos de fotografias). Isto deve-se à qualidade da ligação 3G ou 4G da central do seu alarme. Por isso, tenha paciência!
...
Por predefinição, os comandos não são apresentados no widget. O objetivo é, em seguida, criar um dispositivo virtual para cada sensor. Desta forma, poderá obter as informações relativas à abertura, fecho, temperatura e humidade dos diferentes sensores, bem como controlar remotamente as tomadas ligadas à Verisure a partir do Jeedom.

> **Atenção**
>
> Os estados não são atualizados em tempo real (o que, de momento, é impossível devido à Verisure). Terá de atualizar o estado do alarme através de um cenário para os atualizar ou aguardar o cron30. A personalização do cron (5, 10, 15, 30...) estará disponível numa fase posterior. **No entanto, tenha cuidado para não enviar demasiadas solicitações aos servidores da Verisure, sob pena de ser colocado na lista negra.**
...
| Dispositivo | Comandos criados |
|---|---|
| **Fechadura conectada** | estado / abertura / fecho |

# Painel de controlo

O plugin inclui um widget específico para cada tipo de alarme.
...
> **Atenção**
>
> Este valor de 30 minutos poderá vir a sofrer alterações em função do feedback e dos pedidos dos utilizadores, bem como do número de pedidos autorizados por hora pela Verisure nos seus servidores!
...
Este plugin irá evoluir ao longo do tempo, de acordo com os vossos pedidos e com as possibilidades das APIs da Verisure.

> **Dica**
>
> Pode enviar o seu pedido de melhoria criando um «issue» de «melhoria» no [GitHub](https://github.com/Xav-74/verisure/issues/new).
