...
| Ajuste de la altura de la cuchilla / Ajuste de la velocidad | El último valor ajustado mediante los controles deslizantes se vuelve a utilizar al iniciar un corte |
...
| Ajustar la altura de la lama | Control deslizante en mm, por defecto de 30 a 70 (no disponible en la gama Yuka) |
...
| Iniciar una actividad | Inicia una actividad programada en la aplicación (lista alimentada por la sincronización) |
| Activar o desactivar el modo de hibernación | Activa el modo de hibernación o lo desactiva si ya está activo (véase *Modo de hibernación*) |
...
| Limpieza del suelo | Solo el fondo de la piscina (**FLOOR**) |
...
> **Consejo**
>
> Los 4 modos se corresponden exactamente con los 4 botones de la pantalla de inicio de la aplicación Mammotion. Existen otros modos en el protocolo (línea de flotación, personalizado), pero aún no están disponibles.
...
> **Consejo**
>
> El comando *Activar/desactivar el modo de invierno* invierte el estado: si se ejecuta dos veces, vuelve al estado inicial. En un escenario que automatice la activación del modo de invierno, comprueba primero el comando de información **Modo de invierno** y solo ejecuta el comando de activación si su valor es 0.

Al finalizar el modo de hibernación, los datos se actualizarán en la próxima ejecución de la tarea programada, o de forma inmediata si haces clic en *Actualizar*.
...
**Mi robot ya no se actualiza y sus botones no responden**: comprueba el icono de la barra de título del widget. Un sol indica que el dispositivo está en **modo de hibernación**: el cron lo ignora y sus comandos están bloqueados. Al hacer clic en el icono, vuelve al modo estándar.

**Spino: el modo de limpieza muestra «Ninguno»**: es el comportamiento normal en reposo. Un Spino en reposo no indica ningún modo activo en sus mensajes de estado. Utiliza el comando **Estado** para saber si el robot está trabajando.

**Spino: ¿por qué no hay un botón de «Pausa»?**: el protocolo del robot no lo incluye. Para interrumpir un ciclo, utiliza *Parar y volver a la base de carga*.
