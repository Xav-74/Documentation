...
> **Suggerimento**
>
>Il cloud Mammotion consente una sola sessione per account. Crea un **account secondario** dedicato a Jeedom e condividi con esso i tuoi robot dall'app ufficiale; in caso contrario, l'app mobile verrà disconnessa ogni volta che il demone viene avviato (e viceversa).
...
Il rilevamento dei robot avviene tramite il pulsante **Sincronizza** nella pagina del plugin. Il plugin interroga l'account Mammotion, crea i dispositivi mancanti, ne aggiorna il modello e il firmware, quindi recupera l'elenco delle aree di taglio e l'elenco delle attività.

Eseguitelo dopo il primo avvio del demone, poi ogni volta che aggiungete un robot al vostro account o modificate le impostazioni dei vostri robot, delle vostre zone e/o delle vostre attività dall'applicazione.
...
La pagina dedicata al dispositivo e il relativo widget mostrano una foto corrispondente al modello rilevato. Quando il modello esatto non è disponibile, viene utilizzata quella del modello più simile.
...
| Regolazione della velocità | Cursore in m/s, impostazione predefinita 0,2 → 0,6 (non disponibile nella gamma Yuka) |
| Taglia un'area | Avvia il taglio dell'area selezionata (elenco aggiornato tramite sincronizzazione) |
| Avvia un'attività | Avvia un'attività programmata nell'app (elenco aggiornato tramite sincronizzazione) |
...
> **Suggerimento**
>
> I limiti dei due cursori vengono regolati automaticamente in base alle capacità segnalate dal modello in uso: i valori sopra indicati sono solo valori di riserva.
...
> **Suggerimento**
>
> Le 4 modalità corrispondono esattamente ai 4 pulsanti della schermata iniziale dell'app Mammotion. Nel protocollo sono presenti altre modalità (linea d'acqua, personalizzata), ma non sono ancora disponibili.
...
Quando un robot viene riposto per la stagione, viene spento. La **modalità svernamento** mette questa apparecchiatura in stand-by.
...
- l'**apparecchio rimane visibile** sulla dashboard e conserva la cronologia;
- il **demone continua a funzionare** normalmente per gli altri robot.

Questa impostazione è **specifica per ogni apparecchio**: è possibile mettere in modalità invernale il robot per piscina pur continuando a pulire la piscina.
...
- l'**icona nella barra del titolo del widget**: un fiocco di neve quando il robot è in modalità standard, un sole quando è in modalità invernale. Un clic permette di passare da una modalità all'altra;
- il comando **Attivare la modalità invernale**, utilizzabile in uno scenario.

Il comando info **Messa in letargo** indica lo stato attuale. Viene registrato nella cronologia, il che consente di risalire alle date di inizio e fine della messa in letargo.
...
**Spino: la modalità di pulizia indica «Nessuna»**: si tratta del comportamento normale in stato di riposo. Uno Spino spento non segnala alcuna modalità attiva nei propri messaggi di stato. Utilizzate il comando **Stato** per verificare se il robot è in funzione.
