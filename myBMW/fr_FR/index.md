...
Avant de pouvoir utiliser le plugin, vous devez configurer votre compte utilisateur BMW. Veuillez suivre scrupuleusement les étapes suivantes :
...
3. Cliquez sur le bouton **"Créer un client CarData"**
4. Copiez **l'ID client** en lieu sûr !
...
13. Sélectionnez **"Toutes les catégories"** (Vehicle Status, Charging, Trip Data, etc.) puis cliquez à plusieurs reprises sur le bouton **Charger** pour afficher tous les attributs
14. Sélectionnez **manuellement** les 244 attributs individuels ou appuyez sur F12 et entrez dans la **console développeur** et tapez : (pas de copié-collé possible)

    ```js
    document.querySelectorAll('label.chakra-checkbox:not([data-checked])').forEach(l => l.click());
    ```

15. Puis **sauvegardez** votre configuration
16. Copiez **le username** en lieu sûr !
...
> **Attention**
>
> BMW autorise seulement 50 requêtes par jour donc ne rafraichissez pas manuellement trop souvent (le CRON actuel utilise 24 requêtes par jour) au risque d'être bloqué jusqu'au lendemain.
