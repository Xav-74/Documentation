...
3. Klicken Sie auf das Symbol **BMW CarData**

![CarData](../images/CarData_section.png)

4. Klicken Sie auf die Schaltfläche **„CarData-Kunde anlegen“**
5. Notieren Sie sich **die Kunden-ID** an einem sicheren Ort!
4. Bitte **30 Sekunden** warten
...
12. Wählen Sie die 244 einzelnen Attribute **manuell** aus oder drücken Sie F12, um die **Entwicklerkonsole** zu öffnen, und geben Sie Folgendes ein: (Kopieren und Einfügen ist nicht möglich)
```
document.querySelectorAll('label.chakra-checkbox:not([data-checked])').forEach(l => l.click());
```
14. Speichern Sie anschließend Ihre Konfiguration
15. Notieren Sie sich **den Benutzernamen** an einem sicheren Ort!
...
-   **VIN**: Geben Sie die VIN-Nummer oder Vehicle Identification Number (Fahrzeugidentifikationsnummer) an. Diese Nummer finden Sie in Feld E Ihres Fahrzeugscheins. Die Nummer besteht aus 17 Zeichen.
...
-   **Farbe der Tür- und Fenstersymbole**: Wenn Sie den Symbolmodus gewählt haben, können Sie auch die Farbe der Symbole festlegen (grün oder schwarz-weiß).
-   **Wohnort (Anwesenheit)**: Sie haben drei Möglichkeiten, die GPS-Koordinaten Ihres Wohnorts anzugeben: Entweder verwenden Sie die in Jeedom hinterlegten Koordinaten, nutzen die aktuellen Koordinaten des Fahrzeugs oder geben den Breiten- und Längengrad manuell ein.
...
-   **Status aller Türen**
-   **Status des vorderen Fahrerscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheibenscheib
...
-   **Anleitung zum Aufpumpen des linken Vorderreifens**
-   **Reifendruck vorne rechts**
-   **Anleitung zum Aufpumpen des rechten Vorderreifens**
-   **Reifendruck hinten links**
-   **Anleitung zum Aufpumpen des linken Hinterreifens**
-   **Reifendruck hinten rechts**
-   **Anleitung zum Aufpumpen des rechten Hinterreifens**
...
-   **Verbleibende Reichweite** (nur bei Elektro- oder Hybridfahrzeugen)
-   **Verbleibende Reichweite (elektrisch)** (Elektromotor und/oder Hybridantrieb)
-   **Verbleibende Kraftstoffmenge** (nur bei Fahrzeugen mit Verbrennungsmotor oder Hybridantrieb)
...
-   **Ladevorgänge**: Zeigt die Statistiken zum Ladeverbrauch der Batterien im laufenden Monat an (nur bei Elektro- oder Hybridfahrzeugen)
-   **Gesamtstromverbrauch**: Gibt den im laufenden Monat verbrauchten Gesamtstrom an (gilt nur für Elektro- oder Hybridfahrzeuge)
-   **Gesamtstromkosten**: Gibt die monatlichen Stromkosten an
-   **Ladeziel**: Zeigt das für das Fahrzeug konfigurierte Ladeziel an (in %)
-   **Ladestrombegrenzung**: Gibt die Obergrenze des Ladestroms an, der verwendet wird, wenn das Fahrzeug angeschlossen ist
-   **Ladestrombegrenzung**: Gibt Auskunft darüber, ob eine Ladestrombegrenzung am Fahrzeug aktiviert ist oder nicht.
...
-   **Status „Entsperren“**: Gibt den zuletzt bei einer Entsperranfrage empfangenen Status zurück. Wenn die Aktion erfolgreich ausgeführt wurde, wechselt der Status von „PENDING“ zu „EXECUTED“. Andernfalls wird „ERROR“ zurückgegeben.
-   **Status „Sperren“**
-   **Status: Laden**
...
-   **Laden**: Hiermit wird der Ladevorgang des Fahrzeugs gestartet
-   **Ladevorgang unterbrechen**: Hiermit kann der Ladevorgang des Fahrzeugs unterbrochen werden
-   **Lüften (Start)**: Startet die Lüftungsanlage auf die im Fahrzeug eingestellte Temperatur
-   **Belüftung (Stopp)**: Hiermit lässt sich die Belüftungsanlage bei der im Fahrzeug eingestellten Temperatur ausschalten.
-   **Lichthupen**: Löst eine Lichthupen-Aktion aus
-   **Hupen**: Löst einen akustischen Alarm aus
-   **Suchen**: Ortet das Fahrzeug in Echtzeit und zeigt es auf einer Google Maps-Karte an
-   **POI senden**: Senden eines Sonderziels (POI) an das Fahrzeug-Meldungszentrum durch Eingabe des Namens sowie der Breiten- und Längengrade des Ortes
...
Zudem wird ein anpassbarer CRON-Job eingerichtet (alle 2 Stunden, 5 Minuten nach der vollen Stunde), um nicht per Stream übertragbare Informationen abzurufen (Steuerungs- und Servicemeldungen, Ladeverlauf). Achten Sie darauf, diese Frequenz nicht zu stark zu erhöhen, da sonst die von BMW festgelegten Abfragequoten (50 pro Tag) überschritten werden könnten!
