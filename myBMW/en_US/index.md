...
2. Click the **BMW CarData** icon

![CarData](../images/CarData_section.png)

3. Click the **"Create a CarData Customer"** button
4. Make sure to copy **your customer ID** and keep it in a safe place!
5. Please wait **30 seconds**
...
14. Select the 244 individual attributes **manually**, or press F12 to open the **developer console** and type: (copying and pasting is not allowed)

    ```js
    document.querySelectorAll('label.chakra-checkbox:not([data-checked])').forEach(l => l.click());
    ```

15. Then **save** your configuration
16. Make sure to write down **the username** somewhere safe!
...
| Field | Description |
|---|---|
| **Device Name** | your vehicle's name |
...
| **Make** | Enter your vehicle's make (BMW or Mini) |
...
| **Port/Window Icon Colors** | If you've selected icon mode, you can also choose the color of the icons (green or black & white). |
| **Home (presence)** | You have three options for specifying the GPS coordinates of your home: either by using the coordinates entered in Jeedom, by using the vehicle's current coordinates, or by manually entering the latitude and longitude. |
| **Maximum Distance (in m)** | Enter the maximum distance in meters between your home and the vehicle for the vehicle to be considered present at your home. |
...
| **Charging Target** | displays the configured electric charging target for the vehicle (in %) |
| **Charging current limit** | specifies the maximum charging current when the vehicle is plugged in |
| **Charging Current Limit** | provides information on whether a charging current limit is enabled or disabled for the vehicle |
...
| **Lock** | allows you to lock the car remotely |
...
| **Search** | locates the vehicle in real time and displays it on a Google Maps map |
...
> **Please note**
>
> BMW limits requests to 50 per day, so don't manually refresh too often (the current CRON job uses 24 requests per day), or you risk being blocked until the next day.
