...
3. Fare clic sull'icona **BMW CarData**

![CarData](../images/CarData_section.png)

4. Fare clic sul pulsante **"Crea un cliente CarData"**
5. Salva **l'ID cliente** in un luogo sicuro!
...
12. Selezionare **manualmente** i 244 attributi singoli oppure premere F12 per accedere alla **console di sviluppo** e digitare: (non è possibile copiare e incollare)
```
document.querySelectorAll('label.chakra-checkbox:not([data-checked])').forEach(l => l.click());
```
14. Quindi **salvate** la vostra configurazione
15. Salva **il nome utente** in un posto sicuro!
...
-   **Nome dell'apparecchiatura**: nome del vostro veicolo
...
-   **VIN**: indicare il numero VIN (Vehicle Identification Number, numero di identificazione del veicolo). Questo numero è riportato nella casella E della carta di circolazione. È composto da 17 caratteri.
...
-   **Colore delle icone porte/finestre**: se avete scelto la modalità icona, potete anche decidere il colore delle icone (verde o bianco e nero).
-   **Residenza (presenza)**: sono disponibili 3 opzioni per indicare le coordinate GPS della propria residenza: utilizzando le coordinate inserite in Jeedom, utilizzando le coordinate attuali del veicolo oppure inserendo manualmente la latitudine e la longitudine.
...
-   **Stato della portiera anteriore del passeggero**
-   **Stato della portiera del passeggero posteriore**
...
-   **Stato del tetto apribile**
-   **Pressione dello pneumatico anteriore sinistro**
...
-   **Carburante rimanente** (solo su veicoli a combustione interna o ibridi)
-   **Chilometri rimanenti (termico)** (motore termico)
-   **Messaggi**: riporta i messaggi visualizzati a bordo del veicolo (controlli e servizi)
-   **Sessioni di ricarica**: riporta le statistiche relative alla ricarica delle batterie nel mese in corso (solo per veicoli elettrici o ibridi)
-   **Carica elettrica totale**: indica la carica elettrica totale utilizzata nel mese in corso (solo su veicoli elettrici o ibridi)
...
-   **Limite di corrente di ricarica**: indica il limite della corrente di ricarica utilizzata quando il veicolo è collegato alla presa
...
-   **Coordinate GPS**: restituisce la posizione del veicolo nel formato "latitudine, longitudine"
-   **Ultimo aggiornamento**: indica la data e l'ora dell'ultimo collegamento tra l'auto e i server BMW
...
-   **Stato: Interruzione della ricarica**
-   **Stato Ventilazione**
...
-   **Stato Ricerca**
-   **Stato invio POI**
-   **Presenza a domicilio**: indica se il veicolo si trova presso il proprio domicilio (1 = presente, 0 = assente) in base alla distanza massima specificata
...
-   **Blocca**: consente di bloccare l'auto da remoto
-   **Sblocco**: consente di sbloccare l'auto a distanza
-   **Ricarica**: consente di avviare la ricarica del veicolo
-   **Interruzione della ricarica**: consente di interrompere la ricarica del veicolo
...
-   **Segnalazione con i fari**: attiva la segnalazione con i fari
-   **Suonare il clacson**: attiva un segnale acustico
-   **Cerca**: individua il veicolo in tempo reale e lo visualizza su una mappa di Google Maps
...
> **Suggerimento**
>
>È possibile inviare una richiesta di miglioramento creando un issue di tipo "enhancement" su [GitHub](https://github.com/Xav-74/myBMW/issues/new).
